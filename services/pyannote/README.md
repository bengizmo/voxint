...
[docs/gpu-contracts.md](../../docs/gpu-contracts.md) (`POST /v1/diarize`,
`GET /healthz`, port **8024**).

## Model policy

**pyannote/speaker-diarization-3.1** on **pyannote.audio 3.1.1**. This is a
deliberate pin, not a lag: the 4.x pipeline (community-1) silently rejects the
...
`models/*.bin` (download the release assets, or fetch from the upstream HF repos
at the pinned revisions); the build fails on a sha256 mismatch.
...
weights download to `/app/models` on first startup, so mount a volume there to
persist them.
...
| `HF_TOKEN` | (none) | Optional; only for a gated `DIARIZER_MODEL_NAME` override |
| `PYANNOTE_CLUSTERING_THRESHOLD` | `0.55` | Below the ~0.70 default, which under-clusters quiet audio to 0 speakers |
