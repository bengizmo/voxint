...
A tiny synthetic dataset that exercises all three scoring-harness commands. The
...
toy vectors (real speaker embeddings are 192-d+); the harness only requires that
a file's vectors agree with its declared `dims`.
...
Six items, nine scoreable slots, deliberately covering every verdict: correct
names (including an alias match, `"D. Fox"` vs truth `"Daniela Fox"` via
`aliases.json`), a wrong person, an over-name where the truth is abstain, missed
names, a correct abstain, and one unscoreable slot.
...
fp_overname: 1, tn: 1, excluded: 1`, slot `accuracy` ≈ 0.556 with a Wilson CI,
duration-weighted variants, and a `risk_coverage` curve (some slots carry
`confidence`). Because `--baseline` is given, the report also carries a `paired`
block: McNemar over 2 discordant slots (both improvements, since the baseline
missed the alias on ep-002 and named the wrong person on ep-006) plus a seeded
...
| ep-002 | curated (host-lee) | `CONFIDENT_HOST_PRESENT` | `ABSTAIN` (`near_tie`) | in ecapa two slots score within the 0.08 margin, so it abstains rather than guess |
| seed-01 | curated (host-dana) | `ABSTAIN` (`session_leakage_risk`) | same | the item is in host-dana's `source_item_ids`, and a voiceprint never judges the item it was built from |
...
(`embedding_space` mismatch). Try it: that refusal is the cross-space invariant
doing its job.

## 3. Ensemble fusion

Fuse the two voters (verdicts only, since the ensemble layer cannot see
vectors):
...
Reports are deterministic: identical inputs (and `--seed`) produce byte-identical
outputs, so the expectations above are stable.
