"""The count-inflected "needs you" chip macro (#318).

The generic ``chip()`` macro is an EXACT-key lookup, so a count-prefixed label
like "2 voices need you" falls through to chip-neutral — which is exactly how
the jobs page shipped a styling bug. ``needs_you_chip`` owns both the
...
_TMPL = (
    '{% from "fragments/_chips.html" import needs_you_chip %}{{ needs_you_chip(n) }}'
)
