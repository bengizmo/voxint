...
class TestSingleZip:
    def test_members_and_byte_identity(
        ...
        run8, ann8 = run_id.hex[:8], ann_id.hex[:8]
        assert (
            resp.headers["content-disposition"]
            == f'attachment; filename="voxint-{run8}-quote-{ann8}.zip"'
        )
        md_name = f"voxint-{run8}-quote-{ann8}.md"
        json_name = f"voxint-{run8}-manifest-{ann8}.json"
...
class TestBulkZip:
    def test_members_and_bundle_identity(
        ...
        expected = sorted(
            [
                f"voxint-{run8}-quote-{first.hex[:8]}.md",
                f"voxint-{run8}-quote-{second.hex[:8]}.md",
        ...
        for ann in (first, second):
            ...
            assert (
                _zip_member(resp.content, f"voxint-{run8}-quote-{ann.hex[:8]}.md")
                == standalone.content
            )
    ...
    def test_tag_filter(
        ...
        assert md_names == [f"voxint-{run8}-quote-{tagged.hex[:8]}.md"]
