...
def test_run_step_banner_renders_on_tutorial_run(
    ...
    assert "tutorial=review" in resp.text
    assert "Open the editor" in resp.text
...
# Tests for tutorial banner rendering on retired review pages (workbench, queue,
# transcript stepper) were removed in issue #158. The tutorial now renders
# banners on the media editor page.
...
# Tests for tutorial step banners on retired review pages (workbench, transcript
# stepper) and htmx fragment banner exclusion (now JSON responses) were removed
# in issue #158. The tutorial flow now renders banners on the media editor page.
...
def test_finish_plain_falls_back_to_media_when_unseeded(
    session_factory: sessionmaker[Session], settings: Settings
) -> None:
    client = _client(session_factory, settings, onboarded=False)
    resp = client.post(
