...
- A discovered latent bug: `compose.darkboi.yaml` comments describe a two-lane
  worker split via Celery `task_routes`, but **no `task_routes` exists in code**
...
- Update `compose.yaml` to run the two lanes (base gains a `post` worker; today
  the split lives only in `compose.darkboi.yaml`), and **correct the misleading
  darkboi comments**.
- **Critical files:** `src/voxint/pipeline/engine.py` (`execute_run`,
  ...
  `src/voxint/worker/app.py` (`task_routes`), `compose.yaml`,
  `compose.darkboi.yaml`.
