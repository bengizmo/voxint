...
password), and click **Settings** in the sidebar. Every option saves right
there, with no files to edit and no services to restart.

Settings are organized into tabs along the top of the page:

- **General**: appearance, feature toggles, the guided tutorial.
- **Media**: your media folders, automatic ingest, and sources and research.
- **AI**: LLM enhancement, corrections, glossary, and semantic search.

Below the tabs, several read-only pages (Status, Hardware, Database, Plugins)
show what is running and how the system is doing. You reach those from the same
tab bar.
...
Each section has its own **Save** button and saves on its own; saving one
...
- **LLM name pass**: additionally asks the enhancement LLM to propose speaker
  names. This one needs both LLM enhancement (on the AI tab) and Speaker name
...
bundled local model** (on the General tab, under Features) instead: it needs no
endpoint URL and no key. It covers enhancement and run-asset summaries + entities
only; keep a configured endpoint here if you also want the LLM name pass or web
research.
...
- **Enhancement won't turn on.** Either no API key is configured (neither a saved
  ...
  transcription stage's lease. The LLM section (on the AI tab) tells you which.
  Enter a key to fix the first; the second is an installation setting your
...
- **"Does nothing leave my machine?"** By default, all processing (transcription,
  ...
  turn it on**: fetching media from a URL (on the General tab, under Features),
  web research (on the Media tab, under Sources and research), and sending
  transcript segments to a remote LLM endpoint (on the AI tab, under LLM
  enhancement, if you point it at a remote model). Leave those off and Voxint
  stays entirely on your machine.

### Checking your hardware

If runs feel slow or keep failing, open the **Status** tab in Settings. At the
top, a banner shows whether everything is running normally, your install type,
GPU acceleration status, and the current version. A **Check for updates** button
...
The **Hardware** tab shows pipeline models: which model each part of the
pipeline is running right now for transcription, speaker diarization, and speaker
embedding. It reads this live from each service as the page loads, so it reflects
what is actually running, not what a config file claims. You cannot change models
here; that is an advanced task covered in [Changing pipeline
models](changing-pipeline-models.md).

The status page speaks up in only two cases, each with one plain fix:
