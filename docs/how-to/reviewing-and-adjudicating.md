...
Voxint listens to your recording and makes its best guesses — who spoke, when,
and what they said. Those guesses are **proposals**, not verdicts. This guide
walks you through the review console, where **you have the final say**: you
...
tab, so only you can make rulings on it while you work — no one else (and no
...
this voice, and each transcript line under it has its own ▶ to play just that
line. Use them to confirm what you're about to decide. (If playback controls are
greyed out, a banner explains why — usually the audio can't be reliably lined up
with the timeline; you can still scrub the main player by hand.)
...
**Accept** or **Reject** each one. Accepting a hint records your review of it —
it does **not** assign a speaker. Assigning is always the explicit **Assign** or
**Enroll new** action.
...
- If the words are right, **Verify** it — Voxint marks it checked and jumps you
  to the next unchecked line.
...
2. The current line's words become clickable. **Click the word where the new
   speaker starts** — Voxint cuts the segment just before it.
