...
There is deliberately **no built-in list**, because what counts as authoritative
differs by deployment. Empty (the default) leaves `source_authority` at 0.0 for every
...
tie-breaking, a version, and regression fixtures showing the full breakdown, all
