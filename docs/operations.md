...
| `GET /activity/events` | Activity bootstrap (JSON): the latest snapshot of run-completion and speaker-identification events plus the live-jobs badge count. Called once on page load to seed the client state before the SSE stream opens. Dark-shipped behind `CONSOLE_ACTIVITY_ENABLED` (answers 404 until on) |
| `GET /activity/stream?since={id}` | Activity live delivery (SSE, `text/event-stream`): streams `init`, `reset`, `activity`, and `badge` events as they occur. Supports `Last-Event-ID` reconnect. Keepalive comments every 15 s. The `since` cursor is the last event id from the bootstrap or a previous stream (#499) |
