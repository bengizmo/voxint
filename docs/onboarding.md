...
| Media folders | `/setup/folders` | A **folder browser** confined to `MEDIA_ROOT`: click into sub-folders and register the ones Voxint should watch, and optionally pick a **domain pack** per folder to tune transcription/enrichment (a "Default" leaves it unmapped). Browsing is re-validated on the server every request, so a typed or crafted path can never escape `MEDIA_ROOT`. The same panel appears under **Settings → Media folders**. An optional **bounded scan** (`/setup/scan`) then previews audio/video not yet known to Voxint and batch-registers it for transcription. |
...
| Text clean-up and name hints | `/setup/llm` | Toggle optional transcript enhancement and set an OpenAI-compatible endpoint/model **and API key**. Best-effort by design: a slow or failing model never blocks a run, and enhancement is skipped. |
| Readiness | `/setup/services` | Live reachability check of the ASR / diarizer / embedder model services (GPU or CPU tier). Advisory only; you can finish regardless. A run submitted while a needed service is down retries with backoff and eventually **fails**; requeue it from the run's page once services are up. |
